    ...
    <UiSection size="md">
      <UiContainer size="md">
        ...
        <UiCard v-else :showFooter="false">
          <template #header>
            <span v-if="isCurrentJob" class="badge bg-primary fs-6 mb-1">Job actuel</span>
          ...
          <div>
            <div v-if="hasShortDescription" class="mb-2 mb-lg-3">
              <h3 class="h4 mb-lg-2"><span class="h3">✍️</span> Description</h3>
              <p class="short-description ps-0 ps-lg-3">
                {{ state.job.item.shortDescription }}
              </p>
            </div>
            <div class="row gx-4">
...
const hasShortDescription = computed(() => {
  return (
    state.job.item.shortDescription !== undefined &&
    state.job.item.shortDescription !== ""
  );
});
...
const hasSuccess = computed(() => {
  if (
    state.job.item.successContent !== undefined &&
...
const hasMissions = computed(() => {
  if (
    state.job.item.missionsContent !== undefined &&
...
const datesSentence = computed(() => {
  ...
  return `entre ${start.toLocaleString("FR-fr", localeOptions)} et ${end.toLocaleString(
    "FR-fr",
    localeOptions
  )}`;
});
...
const fetchJob = () => {
  ...
  resumeService.getSingleJob(route.params.id).then((res) => {
