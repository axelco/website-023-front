<template>
  <UiSection>
    <UiContainer size="sm">
      <SectionTitle :title="`Mes diplômes et certificats`" emoji="🌠" align="center" />
    </UiContainer>
    <UiContainer size="lg">
      <div v-if="state.diplomasApiError !== null && state.diplomasLoaded">
        <div class="alert alert-warning">
          <p class="fs-5">Une erreur est survenue</p>
          <p>{{ state.diplomasApiError }}</p>
          <button class="btn btn-warning" @click="fetchDiplomas">Réessayer</button>
        </div>
      </div>
      <div v-else>
        <div v-if="hasInProgressDiplomas" class="row my-2 my-lg-5">
          <div
            v-for="item in state.inProgessDiplomas"
            :key="item._id"
            :class="`col-12 mb-2 ${
              hasMoreThanOneProgressDiploma ? 'col-md-6' : 'col-md-8 offset-md-2'
            }`"
          >
            <DiplomaItem
              class="h-100"
